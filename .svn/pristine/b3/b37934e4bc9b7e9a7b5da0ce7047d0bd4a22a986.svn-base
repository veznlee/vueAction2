<template>
    <div class="table">
        <div class="container">
            <el-table :data="tableData" border stripe style="width: 100%" v-loading="listLoading" element-loading-text="正在载入数据，请稍候…">
                ...
                <el-table-column prop="remarks" align="center" label="备注" min-width="250"></el-table-column>
            ...
            <el-form :rules="rules" ref="form" :model="form" status-icon label-width="100px">
                ...
                <div class="el-main" style="display:inline-block;width:50%">
                    ...
                    <el-row>
                        ...
                        <el-col :span="12" v-show="ifShowHide">
                            <el-form-item label="隐藏：">
                                <el-checkbox v-model="form.checkShow"></el-checkbox>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="24">
                            <el-form-item label="经办人：" prop="doby">
                                <div>
                                    ...
                                    <div v-for="(dobyItem,index) in form.doby" style="display: inline-block;border:1px solid #c0e2c0;border-radius: 3px;margin-right: 10px;height:30px;line-height: 28px;padding-left: 5px;padding-right: 5px;cursor:pointer">{{dobyItem.name}} <a @click="deleteDobyItem(index)" style="color:#F56C6C">X</a></div>
                                </div>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="24">
                            <el-form-item label="备注：">
                                ...
                                <el-input type="textarea" :rows="3" :cols="15" v-model="form.remark"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </div>
            ...
            <el-form :rules="rules" ref="flowForm" :model="formFormData" status-icon label-width="100px">
                ...
                <fieldset>
                    ...
                    <div v-for="item in formFormData.orderFieldDesignings" class="input-column">
                        <div class="input-title">{{item.name}}</div><el-checkbox v-model="item.isRequired"></el-checkbox> 必填  权限<el-select v-model="item.permission" placeholder="选择权限" class="handle-select ml10">
                ...
                <fieldset>
                    ...
                    <div v-for="item in formFormData.orderHandleDesignings" class="input-block">
                        ...
                        <div v-if="item.name=='agree'">
                            ...
                            <!--<el-checkbox v-model="item.isGeneratePassword" style="margin-left: 20px" label="生成下一步骤密码"></el-checkbox> 密码别名：<el-input type="text" v-model="item.passwordAnotherName" class="input-alias"></el-input>-->
                        </div>
                    </div>
                </fieldset>
            </el-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTableData(selectedProcess) {
                ...
                request({
                    ...
                    if (response.data.code == 0) {
                        ...
                        this.loadMenu(null, '');
                        if(typeof selectedProcess=="number") {
                            request({
                                host: 1,
                                ...
                                if (response.data.code == 0) {
                                    ...
                                    for (let i = 0; i < this.itemsData.length; i++) {
                                        this.itemsData[i].checkShow = (this.itemsData[i].isShow == "1");
                                    ...
                                    this.handleExpand(this.itemsData[selectedProcess], selectedProcess);
            ...
            // 保存编辑
            saveData(selectedProcess) {
                this.$refs['form'].validate((valid) => {
                    if (valid || typeof selectedProcess=="number") {
                        for (let i = 0; i < this.itemsData.length; i++) {
                            let sameNameNum = 0;
                            for (let j = 0; j < this.itemsData.length; j++) {
                                if (this.itemsData[i].name == this.itemsData[j].name)
                                    sameNameNum++;
                            }
                            if (sameNameNum > 1) {
                                this.$message.warning("节点名字\"" + this.itemsData[i].name + "\"重复");
                                return;
                            }
                            this.itemsData[i].assignee = "";
                            this.itemsData[i].department = "";
                            if (this.itemsData[i].checkShow == true) {
                                this.itemsData[i].isShow = 1
                            } else {
                                this.itemsData[i].isShow = 0
                            }
                        ...
                        request({
                            ...
                            if (response.data.code == 0) {
                                this.oData = JSON.stringify(this.itemsData);
                                this.getTableData(selectedProcess);
            ...
            handleExpand(item, idx){
                if (idx + 1 == this.itemsData.length) {
            ...
            addFlow(){
                let newName="新增审核",found=true;
                while(found) {
                    found = false;
                    for (let i = 0; i < this.itemsData.length; i++) {
                        if (this.itemsData[i].name == newName) {
                            found = true;
                            newName = newName + 1;
                        }
                    }
                ...
                this.saveData(this.selectedProcess + 1);
            },
            deleteFlow(){
