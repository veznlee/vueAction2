        ...
        <div class="container">
            <div class="handle-box">
                ...
                <div align="right" class="search-right">
                    <el-input v-model="listParam.condition" placeholder="类别编码/类别名称" @keyup.enter.native="search" class="handle-input"></el-input>
            ...
            <el-table :data="tableData" border style="width: 100%" v-loading="listLoading" element-loading-text="给我一点时间" class="p-table-center" stripe highlight-current-row>
                ...
                <el-table-column prop="cdesc" label="类别描述" min-width="200">
                </el-table-column>
        ...
        <el-dialog :title="formTitle" :visible.sync="editVisible" width="30%" custom-class="mod-el-dialog" :close-on-click-modal="false">
            <el-form :rules="rules" ref="form" :model="form" label-width="80px">
                ...
                <el-form-item label="类别描述" prop="cdesc">
                    <el-input type="textarea" v-model="form.cdesc" :maxLength="100"></el-input>
                </el-form-item>
            </el-form>
        ...
        methods:{
            ...
            isExist(rule, value, callback){
                ...
                request({
                    ...
                    method: 'get',
                    params: {cname:this.form.cname,ctype:'1'}
                }).then(response => {
                    if(response.data.code==0){
                        callback();
                    }else if(response.data.code==100){
                        callback(new Error('类别名称已存在，请重新输入'))
                    }
                })
            },
        }
